...
const QuestionEdit = () => {
  ...
  const GetData = async () => {
    const res = await fetch(`/getquestiondata/${id}`, {
      method: "GET",
      headers: {
  ...
  const updateQuestionData = async (e) => {
    e.preventDefault();

    const { name, subject, year, link } = questionname;

    const res2 = await fetch(`/updatequestion/${id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },

...
const ComplaintManage = () => {
  ...
  const getuser = async(e) =>{
    const res = await fetch("/allcomplaint",{
        method:"GET",
        headers:{
      ...
      <div className="paymentbox bg-white p-36 rounded m-10">
          ...
          <tbody>
            {
              getcomplaintdata.map((element, id)=>{
                if(element.complaintStatus === "Pending" || element.complaintStatus ==="UnResolved"){
                return(
                  <tr className='h-32 border-spacing-4 border ' key={id} >
                    ...
                    <td className='px-10'>{element.complaintStatus}</td>
        ...
        <table className="table-auto text-start w-full  ">
          ...
          <tbody>
            {
              getcomplaintdata.map((element, id)=>{
                if(element.complaintStatus === "Resolved"){
                return(
                  <tr className='h-32 border-spacing-4 border ' key={id} >
                    <td className='px-10'>{element.name}</td>
                    <td className='px-10'>{element.studentid}</td>
